...
export function Section02() {
  ...
  return (
    <div
      ...
      ref={ref}
    >
      <Image src="/img/section02_bg.webp" alt="section 02 background" fill className="absolute top-0 left-0" />
      ...
      <div className="z-10 flex flex-col items-center gap-20 w-full max-w-[1120px]">
        <div className="grid grid-cols-2 w-full gap-6 max-md:grid-cols-1 max-md:gap-[16px] max-md:p-[4px]">
          {STORY_BOX_DATA.map((data) => (
            <OurValueBox key={data.title} title={data.title} imagePath={data.imagePath} content={data.content} />
          ))}
        </div>
      </div>
    </div>
...
function OurValueBox({ title, imagePath }: { title: string; imagePath: string; content: string }) {
  return (
    <div className="relative flex flex-col items-start justify-start gap-1 p-8 w-full aspect-[540/360] rounded-[40px] max-md:min-h-[219px] max-md:rounded-[20px] hover:scale-105 transition-transform duration-300">
      <Image
        src={imagePath}
        alt={''}
        fill
