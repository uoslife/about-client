...
const mapKoreanToEnglish = (koreanCategory: string): CategoryType | null => {
  const koreanToEnglish: { [key: string]: CategoryType } = {
    개발: CategoryEnum.DEVELOP,
    디자인: CategoryEnum.DESIGN,
    마케팅: CategoryEnum.MARKETING,
    PM: CategoryEnum.PM,
    '취업∙인턴': CategoryEnum.EMPLOYMENT,
    대외활동: CategoryEnum.EXTERNAL_ACTIVITY,
  };
  return koreanToEnglish[koreanCategory] || null;
};
...
export default function WritePage() {
  ...
  useEffect(() => {
    if (isEditMode) {
      ...
      if (savedPost) {
        ...
        const mappedCategory = mapKoreanToEnglish(post.category || '');
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    if (!session.session || (!isMoments && !category)) return;
    ...
    const data: CreateArticleInput = {
      ...
      title,
      content,
      category: isMoments ? CategoryEnum.EXTERNAL_ACTIVITY : category!,
      summary,
      thumbnailFile,
    };
